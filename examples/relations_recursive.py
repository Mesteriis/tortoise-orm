...
class Employee(Model):
    ...
    async def full_hierarchy__async_for(self, level=0):
        ...
        """
        text = [
            f'{level * "  "}{self} (to: {", ".join([str(val) async for val in self.talks_to])}) (from: {", ".join([str(val) async for val in self.gets_talked_to])})'
        ]
    ...
    async def full_hierarchy__fetch_related(self, level=0):
        ...
        text = [
            f'{level * "  "}{self} (to: {", ".join([str(val) for val in self.talks_to])}) (from: {", ".join([str(val) for val in self.gets_talked_to])})'
        ]
